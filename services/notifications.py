...
async def send_daily_digest(
    ...
    try:
        ...
        message = (
            ...
            f"👥 <b>Пользователи:</b>\n"
            f"• Новых за день: {stats['new_users']}\n"
            f"• Всего: {stats['total_users']}\n\n"
            f"📦 <b>Активность за день:</b>\n"
            f"• Запросов артикулов: {stats['article_requests']}\n"
            f"• Фото отправлено: {stats['photos_sent']}\n"
            f"• Видео отправлено: {stats['videos_sent']}\n"
        )

        # Воронка (если данные есть)
        if 'funnel_started' in stats:
            message += (
                f"\n📈 <b>Воронка (всего):</b>\n"
                f"• Нажали /start: {stats['funnel_started']}\n"
                f"• Отправили артикул: {stats['funnel_active']}\n"
                f"• Отправили артикул >1 раза: {stats['funnel_returning']}\n"
            )

...
| Database | PostgreSQL + asyncpg | 16 + 0.29.x | Аналитика и пользователи |
| Scheduler | APScheduler | 3.10.x | Ежедневные дайджесты |
...
**0.3.2** — Кеширование найденных видео URLs
...
### Версия 0.4.x — Инфраструктура и деплой

**0.4.0** — Local Bot API и Docker
- Поддержка Local Bot API (лимит 2GB)
- Docker Compose инфраструктура
- PostgreSQL 16 для экосистемы
- Деплой на VPS

### Версия 0.5.0 — Система аналитики и уведомлений (текущая версия)

**Основные возможности:**
- Отслеживание пользователей и активности
- Мгновенные уведомления о новых пользователях в Telegram-канал
- Автоматические ежедневные дайджесты (00:00 МСК)
...
- SQL миграция: `postgres/init/02-analytics.sql`
- Graceful degradation при недоступности БД
- Callback Pattern для трекинга отправки медиа
...
│
├── db/
│   └── connection.py               # Пул PostgreSQL (94 строки) [0.5.0+]
│
├── bot/
│   ├── handlers/
...
│   ├── media_downloader.py         # Загрузка медиа (494 строки)
│   ├── hls_converter.py            # HLS → MP4 (142 строки) [0.2.0+]
│   ├── video_cache.py              # Кеш видео URLs (91 строка) [0.3.2+]
│   ├── analytics.py                # Аналитика (370 строк) [0.5.0+]
│   ├── notifications.py            # Уведомления (134 строки) [0.5.0+]
│   └── digest.py                   # Дайджест (57 строк) [0.5.0+]
│
├── utils/
...
services.notifications
  ├─> config.settings (ANALYTICS_CHANNEL_ID, ENABLE_ANALYTICS)
  └─> aiogram.Bot для отправки сообщений

services.digest
  ├─> services.analytics (AnalyticsService)
  └─> services.notifications (send_daily_digest)
```
...
### 6. AnalyticsService — Система аналитики

**Файл:** [services/analytics.py](services/analytics.py)
**Строк кода:** 370
**Версия:** 0.5.0+

#### Назначение
Отслеживание пользователей, активности бота и сбор статистики для автоматических отчётов.

#### Архитектура БД

**Таблицы PostgreSQL:**
```sql
-- Пользователи (shared.users)
telegram_id BIGINT PRIMARY KEY
username, first_name, last_name VARCHAR
first_seen, last_seen TIMESTAMP

-- События (shared.analytics_events)
id SERIAL PRIMARY KEY
telegram_id BIGINT FK → shared.users
event_type VARCHAR(50)  -- 'user_start', 'article_request', 'photo_sent', 'video_sent', 'error'
event_data JSONB       -- Гибкие данные события
...
```python
async def track_user_start(user_id, username, first_name, last_name) → Optional[bool]:
    """
    Отслеживание запуска бота пользователем.

    Returns:
        True - новый пользователь (INSERT)
        False - повторный пользователь (UPDATE last_seen)
        None - ошибка БД (graceful degradation)
    """

async def track_article_request(user_id, nm_id) → None:
    """Логирование запроса артикула"""

async def track_photos_sent(user_id, nm_id, count) → None:
    """Логирование отправки фото"""

async def track_video_sent(user_id, nm_id) → None:
    """Логирование отправки видео"""

async def track_error(user_id, error_type, error_message) → None:
    """Логирование ошибок"""

async def get_daily_stats(target_date) → Optional[Dict]:
    """
    Статистика за день.
...
Все методы работают даже если БД недоступна - логируют warning и продолжают выполнение.

---

### 7. Notifications — Уведомления в Telegram

**Файл:** [services/notifications.py](services/notifications.py)
**Строк кода:** 134
**Версия:** 0.5.0+

#### Назначение
Отправка уведомлений и дайджестов в аналитический Telegram-канал.

#### Функции

```python
async def send_new_user_notification(bot, user_id, username, first_name) → bool:
    """
...
async def send_daily_digest(bot, stats, target_date) → bool:
    """
    Ежедневный дайджест статистики.

    Формат:
    📊 Статистика за ДД.ММ.ГГГГ
    👥 Пользователи: новых X, всего Y
    📦 Активность: запросов Z, фото N, видео M
    ⚠️ Ошибки: K
    """
```
...
### 8. DB Connection — Пул соединений PostgreSQL

**Файл:** [db/connection.py](db/connection.py)
**Строк кода:** 94
**Версия:** 0.5.0+

#### Назначение
Управление пулом asyncpg соединений с PostgreSQL.
...
async def get_pool() → Optional[asyncpg.Pool]:
    ...
    Returns:
        asyncpg.Pool или None при ошибке (graceful degradation)
    """

async def close_pool() → None:
    """Graceful shutdown пула при остановке бота"""
```

---

### 9. Digest Service — Планировщик дайджеста

**Файл:** [services/digest.py](services/digest.py)
**Строк кода:** 57
**Версия:** 0.5.0+

#### Назначение
Фоновая задача для ежедневной отправки статистики.

```python
async def send_daily_digest_job(bot, target_date=None) → bool:
    """
    Задача для APScheduler.

    Запускается ежедневно в 00:00 МСК.
    По умолчанию отправляет статистику за вчерашний день.
    """
```
...
| test_analytics.py | 13 | AnalyticsService (трекинг, статистика) |
| test_decorators.py | 10 | Декораторы (timing, retry, logging) |
| test_article_handler.py | 8 | Обработчик артикулов |
| test_callbacks_handler.py | 11 | Обработчик кнопок (включая callbacks) |
...
### Среднее время операций (версия 0.5.0)
...
**Wildberries Media Bot 0.5.0** — зрелое асинхронное приложение с:
...
- ✅ Системой аналитики с PostgreSQL
- ✅ Автоматическими уведомлениями о новых пользователях
- ✅ Ежедневными дайджестами (00:00 МСК)
- ✅ 82% покрытием тестами (113 тестов)
- ✅ Детальным логированием
- ✅ Graceful degradation при ошибках

**Производительность:** показ карточки товара за 2-6 сек (было 40 сек в v0.1.x).

**Надёжность:** 113 тестов, автоматические retry, глобальная обработка ошибок.

**Аналитика:** Полное отслеживание активности, мгновенные уведомления, автоматические отчёты.
